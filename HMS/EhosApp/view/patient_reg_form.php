...
if (isset($_POST['submit'])) {
    ...
    if (!empty($username) && !empty($password1) && !empty($password2) && ($password1 == $password2)) {
        ...
        if ($users == false) 
        {
            $pid = add_patient( $name, $gen, $age, $addr, $pno, $email );
            // The username is unique, so insert the data into the database
            add_user_patient( $username, $password1, $pid );

            // Confirm success with the user
            echo '<p>New Patient account has been successfully created.  <a href="login.php">log in</a>.</p>';
            exit();
        } else {
...
<html>
    ...
    <body>
        ...
        <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
            <fieldset style="width: 150pt; background: azure">
                ...
                <label for="gen">Enter Gender:</label><br />
